<template>
    <el-container>
        <h1 class="flex-center  flex_jufy_center">班级管理</h1>
        <el-header>
            <!-- <div style="width: 200px; margin: 0 10px;">
                <el-input placeholder="请输入学校ID" :disabled="schoolsid == '' ? true : false" clearable prefix-icon="el-icon-search" v-model="inpSchools"></el-input>
            </div>
			<el-button type="primary" :disabled="schoolsid == '' ? true : false" @click="searchSchool()">搜索</el-button> -->
			<el-select v-model="schoolsid" clearable placeholder="请选择学校">
			    <el-option v-for="item in tableData" :key="item.schoolId" :label="item.schoolName" :value="item.schoolId"> </el-option>
			</el-select>
            <el-button type="primary" @click="getAllClass()" :disabled="schoolsid == '' ? true : false" v-loading.fullscreen.lock="fullscreenLoading">查询</el-button>
            <el-button type="primary" :disabled="schoolsid == '' ? true : false" @click="addSchools()">添加班级</el-button>
        </el-header>
        ...
        <el-main>
            <el-table :data="tableClass" height="455" stripe border>
				...
				<el-table-column prop="id" label="学校ID" width="80">
				    <template slot-scope="scope">
        ...
        <ConfirmDialog @confirm="confirm" :showMsg.sync="showMsg"  tit="系统提示" :ftit="'确定要删除？'+delateschool.className" type="danger"></ConfirmDialog>
		<condialogfrom @confirm="confirm2" :showMsg.sync="showMsgForm"  tit="添加班级" :listObj="listobj" type="primary"></condialogfrom>
		<condialogfrom @confirm="confirm3" :showMsg.sync="showMsgModify"  tit="修改班级" :listObj="listobj" type="primary"></condialogfrom>
...
export default {
    ...
    methods: {
		...
		getAllClass(){
			if (this.schoolsid!='') {
				this.fullscreenLoading = true;
				this.$api.school.getAllClass({SchoolId:this.schoolsid},(res) => {
			    console.log(res);
				if(res.status == 0){
		...
		delateschooll(classId){
			this.fullscreenLoading = true;
			this.$api.school.deleteClass({'classId':classId},(res) => {
			    console.log(res);
        ...
        confirm() {
			...
			this.delateschooll(this.delateschool.id);
            console.log('执行确认方法');
        },
		confirm2() {
		    // this.$refs.dialog.visible = false;
			...
			const data = {
				...
				className:this.listobj.value,}
			console.log(data);
			this.$api.school.addClass(data,(res) => {
			    console.log(res);
				if (res.status == 0) {
					this.getAllClass();
		...
		confirm3() {
		    ...
		    console.log('执行修改方法');
			console.log(this.listobj);
			this.$api.school.updateClass(this.listobj,(res) => {
			    console.log(res);
				if (res.status == 0) {
					this.getAllClass();
					this.$message.success('添加成功！')
